...
print(f"📋 需要重新生成的图片: {len(failed_images)} 张")
print(f"🎯 目标尺寸: {target_size[0]}x{target_size[1]} (中心裁剪)")
print()
...
for i, filename in enumerate(failed_images, 1):
    ...
    for attempt in range(3):
        try:
            # 使用 landscape_16_9 尺寸请求（能稳定返回真实图片）
            url = api_base + '?' + urlencode({'prompt': prompt, 'image_size': 'landscape_16_9'})
            ...
            img_final.save(output_path, 'WEBP', quality=85)
            
            new_size = os.path.getsize(output_path) / 1024
            print(f"     ✅ 成功! 原图 {w}x{h} → {target_size[0]}x{target_size[1]}, {new_size:.1f}KB")
            successful += 1
            success = True
            break
